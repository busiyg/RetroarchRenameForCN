...
def smart_match(query, choices, threshold):
    ...
    for candidate, token_score, *_ in candidates:
        ...
        partial_weight = 0.35 if len(candidate) >= len(query) * 0.5 else 0.2
        composite_score = (token_score * 0.35 + ratio * 0.25 + partial * partial_weight + token_sort * 0.15) * len_penalty
...
class RenamerApp:
    def __init__(self, master):
        ...
        Label(master, text="(自动清理文件名前缀并匹配CSV)", fg="gray").grid(row=2, column=1, columnspan=2, sticky='e', padx=6)
        ...
        author_button = Button(author_frame, text="奇个旦", fg="blue", cursor="hand2", relief="flat",
    ...
    def run_lpl_converter(self, lpl_path, threshold):
        ...
        try:
            ...
            # 清理文件名：替换下划线为空格，去掉方括号及内容
            cleaned_filename = os.path.basename(lpl_path)
            cleaned_filename = cleaned_filename.replace('_', ' ')
            cleaned_filename = re.sub(r'\[.*?\]', '', cleaned_filename).strip()

            save_path = os.path.join(desktop_folder, cleaned_filename)
            ...
            self.log_write(f"耗时: {elapsed:.1f}s")
            self.log_write(f"新文件已保存到桌面: {os.path.basename(save_path)}，请覆盖到原文件位置才能生效")
            if platform_stats:
                self.log_write("\n使用的CSV统计:")
                for csv_name, count in sorted(platform_stats.items()):
